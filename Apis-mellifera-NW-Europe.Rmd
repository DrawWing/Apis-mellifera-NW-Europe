...
New data collected for this study are available at Zenodo [@machlowska2026]. There are use also data published during earlier studies [@oleksa2022; @tofilski2025]
...
elev <- rast("X:/WorldClim/wc2.1_2.5m_elev.tif")
...
europe_bbox <- st_bbox(c(xmin = xMin, xmax = xMax,
                         ymin = yMin, ymax = 70),
                       crs = st_crs(4326)) # Crop to Europe
...
countryLin_sf <- st_as_sf(countryLin,
...
figMapClass = ggplot() +
  geom_sf(data = dataMap_europe) +
  geom_arc_bar(data = countryLin_proj, aes(x0 = X, y0 = Y, r0 = 0, r = 150000, amount = Freq, fill = lineage), stat = "pie") +
